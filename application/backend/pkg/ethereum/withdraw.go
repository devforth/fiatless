...
func NewWithdrawProcessor(client *ethclient.Client, privKey *ecdsa.PrivateKey) *WithdrawProcessor {
	from, _ := address.NewEthereumAddressFromPrivateKey(privKey)
...
func (w *WithdrawProcessor) TransferETH(ctx context.Context, to common.Address, amount *decimal.Decimal) (*types.Transaction, error) {
	var wg sync.WaitGroup
